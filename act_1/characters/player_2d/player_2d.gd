...
signal struck_rigidbody(body:RigidBody3D)
...
func spin_movement(_delta: float) -> void:
	...
	movement_direction *= SPIN_MOVESPEED
...
@onready var spin_delay_timer: Timer = $spin_delay_timer
...
const POSTCOLLISION_DELAY_TIME: float = 1.5
...
func enter_spin_state() -> void:
	if spin_delay_timer.time_left > 0:
		return
...
func exit_spin_state(force_delay_time: float = 0) -> void:
	state = STATE.free
	animation_player.play("unflip_x")
	sprite_pivot.rotation_degrees.y = 0
	if force_delay_time > 0:
		spin_delay_timer.start(force_delay_time)
...
func rotate_sprite(_delta: float) -> void:
	...
	if is_on_floor() && state == STATE.spin_startup && rotation_speed >= Globals.MAX_SPIN_TARGET_SPEED:
		...
		velocity.x = spin_direction.x * SPIN_MOVESPEED
		velocity.z = spin_direction.z * SPIN_MOVESPEED
		can_spin = false
...
const SPIN_COLLISION_FORCE:float = 850
...
func handle_collisions(_delta: float) -> void:
	...
	for i in get_slide_collision_count():
		...
		if cur.get_collider() is RigidBody3D:
			struck_rigidbody.emit(cur.get_collider())
			exit_spin_state(POSTCOLLISION_DELAY_TIME)
			var negative_normal:Vector3 = -cur.get_normal()
			cur.get_collider().apply_central_impulse(negative_normal * SPIN_COLLISION_FORCE * _delta)
